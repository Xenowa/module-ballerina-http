...
package io.ballerina.stdlib.http.compiler.analyzer;
...
public class CustomAnalysisTask implements AnalysisTask<SyntaxNodeAnalysisContext> {
    ...
    public void perform(SyntaxNodeAnalysisContext context) {
        ...
        FunctionDefinitionNode resourceFunction = (FunctionDefinitionNode) context.node();
        if (resourceFunction.functionName().text().equals(DEFAULT_KEYWORD)) {
           reporter.reportIssue(document, resourceFunction.functionName().location(), 1);
        }
    }
}
